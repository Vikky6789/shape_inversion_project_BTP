...
def compute_4_metrics(pcn_gt, pcn_output):
    ...
    if pcn_gt.shape[0] <= 150:
        ...
        # Ensure tensors are on the correct device before calculating Chamfer Distance
        device = pcn_gt.device
        dist1, dist2 , _, _ = distChamfer(pcn_gt.to(device), pcn_output.to(device))
